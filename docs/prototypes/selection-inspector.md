...
Open `selection-inspector-demo.pocketry.json` through **Bin & project →
...
- **Objects** keeps the pocket and finger-access lists in their own scroll area.
...
- **Move & arrange** docks the existing move, rotation, alignment and distribution
  controls. Selection remains active while switching between inspector tabs,
  object lists, bin settings, and 2D/3D views.
...
On wide screens the list, canvas and inspector occupy three columns. Tablet
widths stack the list above the inspector beside the canvas. Phones and short
landscape screens use a drawer with Objects/settings and Properties panes. The
canvas stays outside the drawer's transforms.
...
mixed selection, scoped edits, arrangement, undo and drawer navigation.
