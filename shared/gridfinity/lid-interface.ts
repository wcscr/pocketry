...
import { hasOverlappingLid, lidBottomMm, overlapLidRimInsetMm, overlapRimCornerRadiusMm, LID_FRICTION_INTERFERENCE_MM } from "./magnetic-lid";
...
/** Narrow anchors separate adjacent cantilevers; larger lids add more beams. */
export const SIDE_SPRING_ANCHOR_MM = 1.6;
export const SIDE_SPRING_TARGET_PITCH_MM = 24;
...
export interface LidInterfaceFrame {
  ...
  bottom: number;
  contactZ: number;
  /** Fins span a side; side-spring windows tile it between narrow anchors. */
  width: number;
}
...
export function lidInterfaceFrames(spec: BinSpec): LidInterfaceFrame[] {
  ...
  for (const angle of [0, 90, 180, 270]) {
    ...
    const sideSprings = spec.lidInterface === "side-springs";
    const count = fins ? 1 : sideSprings ? Math.max(1, Math.round(usable / SIDE_SPRING_TARGET_PITCH_MM))
      : Math.min(8, Math.max(1, Math.floor(usable / 28)));
    ...
    for (let i = 0; i < count; i++) frames.push({
      ...
      bottom: sideSprings ? lidBottomMm(spec) : overlap ? -4.8 : latch ? 1.55 - LATCH_SPRING_HALF_HEIGHT_MM : 0.85,
      contactZ: overlap ? -3.2 : 1.55,
      width: fins ? usable : sideSprings ? step - SIDE_SPRING_ANCHOR_MM
        : latch ? LATCH_WINDOW_WIDTH_MM : INTERFACE_WINDOW_WIDTH_MM,
    });
  }
  return frames;
}
