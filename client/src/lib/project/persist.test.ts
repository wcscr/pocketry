...
describe("current project persistence", () => {
  it("opens, saves, reloads and exports every item in the v9 Airduster project", async () => {
    ...
    expect(migrated).toEqual({ ...airdusterV9, spec: { ...spec, flatBottom: false, magneticLid: false }, schemaVersion: PROJECT_SCHEMA_VERSION });
    ...
    expect(exported).toEqual({ ...airdusterV9, spec: { ...spec, flatBottom: false, magneticLid: false }, schemaVersion: PROJECT_SCHEMA_VERSION, name: "New Airduster Layout" });
  });
