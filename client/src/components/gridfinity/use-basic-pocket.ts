...
export function useBasicPocket({ toBin }: { toBin: (x: number, y: number) => Point | null }) {
  ...
  return {
    ...
    pointerUp(event: PointerEvent<SVGSVGElement>) {
      ...
      if (!end || Math.hypot(event.clientX - current.clientX, event.clientY - current.clientY) <= 4) return true;
