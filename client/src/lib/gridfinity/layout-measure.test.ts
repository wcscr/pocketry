...
describe("2D layout measurement", () => {
  ...
  it("places split targets correctly after translation, rotation, mirroring and unequal scaling", () => {
    ...
    const shape = { outlineMm: [{ outer: OUTLINE[0].outer.map(p => ({ ...p, y: p.y * 2 - 10 })), holes: [] }] };
    const paths = placedPocketSplitBoundaries([cutout, { ...cutout, split: undefined }], new Map([["tool", shape]]));
    ...
    const extended = { outlineMm: [{ outer: shape.outlineMm[0].outer.map(p => ({ ...p, y: p.y * 1.4 })), holes: [] }] };
    const newPaths = placedPocketSplitBoundaries([cutout], new Map([["tool", extended]]));
    expect(newPaths[0][0].x).toBeCloseTo(14.2);
    expect(newPaths[0][1].x).toBeCloseTo(-8.2);
    const beyondOldEnd = snapToToolContour({ x: 12, y: -4.8 }, [], 1, newPaths)!.point;
    expect(beyondOldEnd.x).toBeCloseTo(12);
    expect(beyondOldEnd.y).toBeCloseTo(-5);
    expect(placedPocketSplitBoundaries([cutout], new Map())).toEqual([]);
  });
});
