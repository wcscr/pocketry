    ...
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain" data-testid="object-list-scroll">
      {sections.map(child => {
        ...
        return <button key={id} type="button" data-testid={`workflow-section-${id}`} data-property-tone={tone}
          className="property-heading flex min-h-16 w-full items-center gap-2 border-b border-l-2 px-3 py-2 text-left hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring aria-pressed:border-l-4 aria-pressed:bg-accent/60"
          ...
          <span className="min-w-0 flex-1"><span className="block text-sm font-medium">{title}</span><span className="block truncate text-xs text-muted-foreground">{summary}</span></span>
